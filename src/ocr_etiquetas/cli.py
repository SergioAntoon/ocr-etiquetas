...
def _argumentos_comunes(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("carpeta", type=Path, help="Folder containing label images")
    parser.add_argument(
        "-s", "--salida", type=Path, required=True, help="Output CSV results file"
    )
    parser.add_argument("--prefijo", default="4260", help="Known code prefix")
    parser.add_argument("--longitud", type=int, default=18, help="Total code lenght")
    parser.add_argument(
        "--validar-sscc",
        action="store_true",
...
def construir_parser() -> argparse.ArgumentParser:
    ...
    extraer = subcomandos.add_parser(
        "extraer", help="Process an entire folder of images with resume support"
    )
    ...
    extraer.add_argument("--escala", type=int, default=2, help="Image scaling factor")
    extraer.add_argument("--contraste", type=float, default=1.5, help="Contrast enhancement")
    extraer.add_argument(
        "--reintentar-fallidos",
        action="store_true",
    ...
    rescate = subcomandos.add_parser(
        "rescate", help="Aggressive OCR pass over selected images"
    )
...
def _comando_extraer(args: argparse.Namespace) -> int:
    ...
    with AlmacenResultados(args.salida, args.reintentar_fallidos) as almacen:
        ...
        if omitidas:
            logger.info("Resuming: skipping %d already processed files", omitidas)
    ...
    logger.info("Finished. %d codes saved to %s", total, args.salida)
    return 0
    ...
    with AlmacenResultados(args.salida) as almacen:
        ...
        for imagen in objetivos:
            ...
            try:
                ...
                logger.exception("Error proccessing %s", imagen.name)
                continue

            almacen.guardar(imagen.name, codigos, metodo="rescate", texto_ocr=texto[:120])
            if codigos:
                rescatados += 1
                logger.info("Recovererd %s -> %s", imagen.name, ", ".join(codigos))
            else:
                logger.warning("Failed on %s. OCR read: %r", imagen.name, texto[:60])

    logger.info("Rescue completed. %d images recovered in %s", rescatados, args.salida)
    return 0
